...
namespace sigyn_lidar_v2 {
  ...
  bool LD06Driver::process_packet(const LD06Packet& packet) {
    ...
    for (int i = 0; i < LD06_POINT_PER_PACK; ++i) {
      ...
      // Debug: Print some raw data to check values
      if (stats_.packets_received % 100 == 0 && i == 0) {  // Print occasionally
        std::cout << "Debug: Raw distance=" << packet.point[i].distance
          << "mm, converted=" << (packet.point[i].distance / 1000.0f)
          << "m, angle=" << angle << "°" << std::endl;
      }
  ...
  void LD06Driver::finalize_scan() {
    ...
    scan.motor_speed = motor_speed_; // new field
