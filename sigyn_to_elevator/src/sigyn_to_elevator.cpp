...
class SigynToElevator : public rclcpp::Node {
 public:
  SigynToElevator() : Node("sigyn_to_elevator"), serial_fd_(-1) {
    // Initialize serial connection
    if (!initSerial("/dev/teensy_gripper", 921600)) {
    ...
    cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
        "/cmd_vel_gripper", 10,
        std::bind(&SigynToElevator::cmdVelCallback, this, std::placeholders::_1));
    ...
    teensy_diagnostics_pub_ =
        this->create_publisher<std_msgs::msg::String>("teensy_elevator_diagnostics", 10);
    ...
    sd_getdir_service_ =
        this->create_service<sigyn_interfaces::srv::TeensySdGetDir>(
            "teensy_elevator_sd_getdir",
            std::bind(&SigynToElevator::handleSdGetDirRequest, this,
    ...
    sd_getfile_service_ =
        this->create_service<sigyn_interfaces::srv::TeensySdGetFile>(
            "teensy_elevator_sd_getfile",
            std::bind(&SigynToElevator::handleSdGetFileRequest, this,
                      std::placeholders::_1, std::placeholders::_2),
            rclcpp::ServicesQoS(),
            service_callback_group_);
    ...
    serial_timer_ = this->create_wall_timer(
        ...
        std::bind(&SigynToElevator::serialCommunicationLoop, this));

    RCLCPP_INFO(this->get_logger(), "Teensy to elevator node started");
  }
...
int main(int argc, char* argv[]) {
  ...
  auto node = std::make_shared<SigynToElevator>();
