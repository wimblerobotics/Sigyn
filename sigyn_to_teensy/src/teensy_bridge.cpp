...
public:
    TeensyBridge() : Node("teensy_bridge"), serial_fd_(-1)
    {
        ...
        odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>(
            "wheel_odom", 10);
        roboclaw_status_pub_ = this->create_publisher<std_msgs::msg::String>(
            "roboclaw_status", 10);
        teensy_diagnostics_pub_ = this->create_publisher<std_msgs::msg::String>(
            "teensy_diagnostics", 10);
...
private:
    bool initSerial(const std::string& device, int baud_rate)
    {
        ...
        if (serial_fd_ < 0) {
            RCLCPP_ERROR(this->get_logger(), "Failed to open serial device: %s", device.c_str());
            return false;
        }
