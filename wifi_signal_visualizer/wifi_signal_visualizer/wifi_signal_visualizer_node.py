...
class WifiSignalVisualizerNode(Node):
    def __init__(self):
        ...
        self.costmap = np.full((self.grid_size, self.grid_size), -1, dtype=np.int8)
...
def main(args=None):
    ...
    for i in range(100):
        x_index = int(random.randint(0, node.grid_size - 1))
        y_index = int(random.randint(0, node.grid_size - 1))
        signal_strength = random.randint(0, 100)
        print(f'x_index: {x_index}, y_index: {y_index}, signal_strength: {signal_strength}')
        if 0 <= x_index < node.grid_size and 0 <= y_index < node.grid_size:
            node.costmap[y_index, x_index] = signal_strength
