...
class PerimeterRoamer(Node):
    ...
    def wait_for_transforms(self):
        ...
        try:
            # Use the appropriate time source based on clock type
            if self.clock_type == rclpy.clock.ClockType.ROS_TIME:
                # Simulation time - use current ROS time
                current_time = self.get_clock().now()
            else:
                # Wall time - use current wall time
                current_time = self.get_clock().now()
            ...
            # Cache the transform
            self.transform_cache['odom_to_base'] = transform
    ...
    def classify_space(self):
        ...
        # Classify space type
        if space_width < self.very_narrow_threshold:
    ...
    def get_distance_in_direction(self, direction_x, direction_y, max_range=10.0):
        ...
        # Check if scan data is in the expected frame
        if self.scan_data.header.frame_id != self.base_frame:
            self.get_logger().warn(f"LaserScan frame mismatch: expected {self.base_frame}, got {self.scan_data.header.frame_id}")
            # In a real implementation, you might want to transform the scan data here
            # For now, we'll assume the scan data is already in the correct frame
