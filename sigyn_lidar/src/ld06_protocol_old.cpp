...
static inline uint8_t crc8_calc(const uint8_t* data, size_t len) { uint8_t crc=0; for(size_t i=0;i<len;i++) crc = CRC_TABLE[(crc ^ data[i]) & 0xFF]; return crc; }

bool LD06Parser::parseBytes(const uint8_t* data, size_t len, double now_sec) {
  stats_.bytes += len;
  buffer_.insert(buffer_.end(), data, data+len);
  while(tryExtractOnePacket(now_sec));
  return hasCompleteFrame();
}

bool LD06Parser::tryExtractOnePacket(double now_sec) {
  // Find header like ldlidar does - scan for 0x54 followed by 0x2C or variant
  while(buffer_.size() >= 2) {
    size_t start = 0;
    while(start < buffer_.size() - 1) {
      if(buffer_[start] == HEADER && 
         (buffer_[start + 1] == 0x2C || buffer_[start + 1] == 0xEC)) {
        break;
      }
      start++;
    }
    ...
    // Use ldlidar's CRC validation approach
    bool crc_valid = true;
    if(!skip_crc_) {
      ...
      // Temporarily skip CRC for testing frame detection
      crc_valid = true;
    }
...
void LD06Parser::addPacketBeams(const LD06PacketRaw& pkt, double now_sec) {
  uint16_t start_a = pkt.start_angle; uint16_t end_a = pkt.end_angle;
  
  // Debug: log first packet details
  static int packet_count = 0;
  if(packet_count < 3) {
    printf("DEBUG: Packet %d - start_angle=%d, end_angle=%d, speed=%d, timestamp=%d\n", 
