...
I achieved it but it was not easy. I wanted to share some lessons learned along the way as well as talk about how this first pass solution works.

## A couple of general observations.

- It is a well known and very hard problem in computer science to name things correctly. This also applies to naming Behavior Tree nodes and blackboard variables. Spend time thinking about the names you use. It will pay off later.
- Having a fast processor will extend your lifespan. 
...
That first approach was to be a somewhat coarse approach. Then I needed to fine tune the position of the robot and get the gripper aligned with the can. This was done by using the Pi camera mounted on the gripper to look down at the gripper and, while slowly raising the arm holding the gripper, the robot would look for the can in the picture once the can finally showed up, and stop when the vertical center of the can was aligned with the vertical center of the gripper. There is not yet any code to deal with any failure here.
...
This all has a myriad of problems that you wouldn't naively think of at first. Here are some of the problems I encountered along the way.
...
ROS code pretty much always has to use transforms when dealing with any kind of position or orientation data. Getting the required transform can take an awfully long time the first time you request it. You have to be prepared to deal with the delay in getting transforms. You have to make sure that your computations are in the correct frame of reference for all the terms involved. A few times I thought the pose of an object was relative to the floor when it wasn't.

Between URDF files, SDF files, config files, and code files, you have to make sure that all of the parameters are consistent. If you change a parameter in one place, you have to make sure it is changed in all the other places. ROS doesn't naturally let you define interesting values in one place, it's up to you to be aware of all the places you need to fix things. Eventually I will solve this by using the M4 macro processor to generate all of the files from a single source of truth.
...
There are a bunch of "gotchas" when using BehaviorTree.CPP that I ran into along the way. At least one node didn't work as advertised, at least in my mind, so I had to create a couple of my own custom reactive control and decorator nodes. I kept forgetting that subtrees do not share blackboard data unless specified with __shared_blackboard="true". I also create a custom SaySomething node that would print out the current pose of the robot and other useful debug information to help me understand what was going on.

While the current behavior tree works reasonably well in simulation, there are still a lot of work to be done. I've turned off the reactive safety tests for the moment. The test for, e.g.:
...
Debugging Behavior Trees is hard. There are a lot of little details that can go wrong. I found that I needed to create a custom node that would present a user interface where I could use behavior injection to simulate hardware failures. This allowed me to test the behavior of the robot when, for example, the robot was about to fall over or the battery was low.
...
- Optical stops for elevator
- Stepper motors for elevator and extender
- Gripper fingers
- Proper physics parameters for all objects
- Simulating joint states so gazebo and rviz will show simulation
- Simulating TF for stepper motors and such
- Pi camera position and tilt to get proper image
- Gazebo/ROS bridging
- No 'mimic' support in gazebo
- Remember that camera simulation needs the camera info topic to work properly in rviz.
- Dealing with getting image data, with its own x,y,z frame of reference, which is different from the ROS frame of reference.
- Crafting  hardware simulation to use dependency injection and mock interfaces so that the real hardware can use the same behavior tree nodes.
...
I need the Macintosh support because, well, frankly, I was spending 16 hours a day (more or less) working on this project and, in the evening, I needed a comfortable environment to work in and that means I need to use my Macbook Pro. My robot and my main ROS workstation both use AMD processors but in the evening I need to move to another room and there my Macbook Pro uses the Apple Silicon instruction set.