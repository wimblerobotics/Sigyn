...
Looking back again at the `VisuallyAquireCan` node and its expansion, we again use a reactive fallback node with a guard test to make sure that we only approach the can once. 
...
The database gives a pose where the expectation is that the OAKD camera should see a can of beer if one is there.

In fact, the OAKD has a fairly narrow field of view. If the can is even once inch outside of that field of view, no attempts is made to look around a bit to see if the can is nearby. And even if the can is seen, there is a simple attempt to get near the can--there is no recovery if, for some reason, there is something in the way. A real behavior tree will have to deal with possible failures and provide recovery behavior.

![VisuallyAquireCan](../media/visually_aquire_can.png)
...
All that is implemented here is a typical backchain design pattern. The reactive fallback node has as its first child a condition node that wants to check for the desired state and the second child creates the desired state. The
There is a custom node, `CanDetectedByOAKD` that will return SUCCESS if the object detector for the OAKD camera sees the can. If not, the second child node will rotate the robot 30 degrees, up to 12 times. This is not sufficient for a read behavior tree but, again, it suffices for the challenge. The assumption is that we have moved to where the can should be visible. 
...
Since we already know that the OAKD camera can see the can, all we do here is rotate the robot
