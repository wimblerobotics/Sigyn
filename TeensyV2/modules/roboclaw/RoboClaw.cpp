...
#define SetDWORDval(arg)                                              \
  (uint8_t)(((uint32_t)arg) >> 24), (uint8_t)(((uint32_t)arg) >> 16), \
      (uint8_t)(((uint32_t)arg) >> 8), (uint8_t)arg
...
bool RoboClaw::SpeedAccelM1M2(uint8_t address, uint32_t accel, uint32_t speed1,
                              uint32_t speed2) {
...
bool RoboClaw::SpeedAccelM1M2_2(uint8_t address, uint32_t accel1,
  ...
  return write_n(18, address, MIXEDSPEED2ACCEL, SetDWORDval(accel1),
                 SetDWORDval(speed1), SetDWORDval(accel2), SetDWORDval(speed2));
}
...
bool RoboClaw::DutyAccelM1M2(uint8_t address, uint16_t duty1, uint32_t accel1,
                             uint16_t duty2, uint32_t accel2) {
  return write_n(14, address, MIXEDDUTYACCEL, SetWORDval(duty1),
                 SetDWORDval(accel1), SetWORDval(duty2), SetDWORDval(accel2));
}

bool RoboClaw::ReadM1VelocityPID(uint8_t address, float &Kp_fp, float &Ki_fp,
                                 float &Kd_fp, uint32_t &qpps) {
  uint32_t Kp, Ki, Kd;
...
bool RoboClaw::ReadM2VelocityPID(uint8_t address, float &Kp_fp, float &Ki_fp,
                                 float &Kd_fp, uint32_t &qpps) {
  uint32_t Kp, Ki, Kd;
...
bool RoboClaw::SetM1PositionPID(uint8_t address, float kp_fp, float ki_fp,
                                float kd_fp, uint32_t kiMax, uint32_t deadzone,
                                uint32_t min, uint32_t max) {
  uint32_t kp = kp_fp * 1024;
  uint32_t ki = ki_fp * 1024;
  uint32_t kd = kd_fp * 1024;
  return write_n(30, address, SETM1POSPID, SetDWORDval(kd), SetDWORDval(kp),
                 SetDWORDval(ki), SetDWORDval(kiMax), SetDWORDval(deadzone),
                 SetDWORDval(min), SetDWORDval(max));
}
...
bool RoboClaw::SetM2PositionPID(uint8_t address, float kp_fp, float ki_fp,
  ...
  uint32_t ki = ki_fp * 1024;
  uint32_t kd = kd_fp * 1024;
  return write_n(30, address, SETM2POSPID, SetDWORDval(kd), SetDWORDval(kp),
                 SetDWORDval(ki), SetDWORDval(kiMax), SetDWORDval(deadzone),
                 SetDWORDval(min), SetDWORDval(max));
}

bool RoboClaw::ReadM1PositionPID(uint8_t address, float &Kp_fp, float &Ki_fp,
...
bool RoboClaw::ReadM2PositionPID(uint8_t address, float &Kp_fp, float &Ki_fp,
                                 float &Kd_fp, uint32_t &KiMax,
                                 uint32_t &DeadZone, uint32_t &Min,
                                 uint32_t &Max) {
  uint32_t Kp, Ki, Kd;
...
bool RoboClaw::SpeedAccelDeccelPositionM1(uint8_t address, uint32_t accel,
  ...
  return write_n(19, address, M1SPEEDACCELDECCELPOS, SetDWORDval(accel),
                 SetDWORDval(speed), SetDWORDval(deccel), SetDWORDval(position),
                 flag);
}
...
bool RoboClaw::SpeedAccelDeccelPositionM1M2(uint8_t address, uint32_t accel1,
  ...
  return write_n(35, address, MIXEDSPEEDACCELDECCELPOS, SetDWORDval(accel1),
...
bool RoboClaw::SetPinFunctions(uint8_t address, uint8_t S3mode, uint8_t S4mode,
                               uint8_t S5mode) {
...
bool RoboClaw::ReadNVM(uint8_t address) { return write_n(2, address, READNVM); }
