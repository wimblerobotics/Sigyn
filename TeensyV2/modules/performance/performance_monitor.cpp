...
void PerformanceMonitor::SendPerformanceReport() {
  ...
  SerialManager::GetInstance().SendMessage("PERF_REPORT", report_msg);
}
...
void PerformanceMonitor::setup() {
  ...
  SerialManager::GetInstance().SendMessage("INFO", "PerformanceMonitor initialized");
}

bool PerformanceMonitor::UpdateConfiguration(const PerformanceConfig& config) {
  if (ValidateConfiguration(config)) {
    config_ = config;
    SerialManager::GetInstance().SendMessage("INFO", "Performance config updated");
    return true;
  } else {
    SerialManager::GetInstance().SendMessage("ERROR", "Invalid performance config received");
    return false;
  }
}

bool PerformanceMonitor::ValidateConfiguration(const PerformanceConfig& config) const {
  return config.min_loop_frequency_hz > 0 &&
         config.max_module_time_ms > 0 &&
         config.max_violation_count > 0 &&
         config.max_frequency_violations > 0 &&
         config.stats_report_interval_ms > 0;
}
