...
SafetyCoordinator::SafetyCoordinator() : current_state_(SafetyState::NORMAL) {
  ...
  if (Module::isAnyModuleUnsafe()) {
    ...
    for (uint16_t i = 0; i < Module::getModuleCount(); ++i) {
      ...
      if (mod && mod->isUnsafe()) {
        ...
        activateEstop(EstopSource::UNKNOWN,
...
SafetyState SafetyCoordinator::getSafetyState() const { return current_state_; }

bool SafetyCoordinator::isUnsafe() { return current_state_ == SafetyState::EMERGENCY_STOP; }
...
void SafetyCoordinator::sendStatusUpdate() {
  char status_msg[128];
  snprintf(status_msg, sizeof(status_msg), "{\"state\":%d,\"estop_active\":%d,\"source\":\"%s\"}",
           static_cast<int>(current_state_), estop_condition_.active, estop_condition_.description.c_str());
...
void SafetyCoordinator::triggerEstop(EstopSource source, const String& description) {
